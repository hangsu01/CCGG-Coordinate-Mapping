...
def load_anchor_info(CC):
    df_019 = pd.read_csv(CC, index_col = None)
    ...
    for chromo in chrlist:
        ...
        CC_dict[chromo] = dict(data[['Anchor', 'Pos']].values)
        ref_dict[chromo] = dict(data[['Anchor', 'Pos38']].values)
    return ref_dict, CC_dict
...
def get_alt_kmer(alt_seq, kmerlist, k):
    alt_kmer = {}
    #alt_seq = seq_37
...
def get_alignment(p_dict, seq_19, seq_37, m):
    ...
    for s in ref_poslist:
        ...
        assert ref_end > ref_start and alt_end > alt_start, "Empty String"

        ref_seq = seq_19[ref_start:ref_end]
        alt_seq = maskNs(seq_37[alt_start:alt_end])
        
        assert len(ref_seq) <= m, "Ref seq too long"
        assert len(alt_seq) <= m, "Alt seq too long"

        #cigar = GapOpenAligner(ref_seq,alt_seq)
        if len(ref_seq) == len(alt_seq):
            c = makeCigar(alt_seq, ref_seq) 
        else:
            c = GapOpenAligner(ref_seq, alt_seq)
            
        var = split(c)

        assert valid_size(var,alt_seq,ref_seq), "Size not match"
        assert valid_eqaul_and_mismatch(var, alt_seq, ref_seq), "Matches and Mismatches"
    ...
    var = split(cigar)
    #print var
    assert valid_size(var,seq_37,seq_19), "Overal Size not Match"
    assert valid_eqaul_and_mismatch(var, seq_37, seq_19), "Matches and Mismatches"
...
def main():
    ...
    parser.add_argument("-a", "--Anchors", type = str, nargs = 1,
                        metavar = ('anchorfile'), help = "anchor_info file for CC genome", required = True)
    parser.add_argument("-g", "--Genomes", type = str, nargs = 2,
                        metavar = ('ref genomefile1','genomefile2'), help = "genome fasta file for reference and CC genome", required = True)
    ...
    ref_dict, CC_dict = load_anchor_info(CC)


    # ref 019, alt 037
    refgenome = args.Genomes[0]
    CCgenome = args.Genomes[1]
    headers_ref, sequences_ref = loadFasta(refgenome)
    headers_cc, sequences_cc = loadFasta(CCgenome)
    print(len(sequences_ref), len(sequences_cc))
    ...
    keys = sorted(ref_dict[chromo].keys())
    keys.remove("SOURCE")
    #keys = ['SOURCE'] + keys
    genome_ref = "+" + sequences_ref[chromo-1].upper()
    genome_cc = "+" + sequences_cc[chromo-1].upper()
    print(len(genome_ref), len(genome_cc))

    #Cigars[chromo] = {}
    outputfilename = args.Output[0]        
    with open(outputfilename + 'pairwisealignment_%s.txt' % str(chromo), 'w') as outfile:
        outfile.write("Anchor, Cigar\n")
        
    for i in range(len(keys)-1):
        
        sanchor = keys[i]
        eanchor = keys[i+1]
        spos_ref = ref_dict[chromo][sanchor] + 45
        epos_ref = ref_dict[chromo][eanchor]
        seq_ref = genome_ref[spos_ref:epos_ref]
        

        spos_cc = CC_dict[chromo][sanchor] + 45
        epos_cc = CC_dict[chromo][eanchor]
        seq_cc = genome_cc[spos_cc:epos_cc]
        
        print(i, sanchor, eanchor, len(seq_ref), len(seq_cc))

        alt_seq = maskNs(seq_cc) # mask Ns
        ref_seq = seq_ref
        if ref_seq == alt_seq.upper():
            cigar = str(len(ref_seq)) + '='
            print(cigar)
            with open(outputfilename + 'pairwisealignment_%s.txt' % str(chromo), 'a') as outfile:
                outfile.write("%s, %s\n" % (sanchor, cigar))                
            continue
        ...
        if len(ref_seq) > maxlength or len(alt_seq) > maxlength:
            ...
            alt_kmer = get_alt_kmer(alt_seq, kmerlist, k)
            ...
            try:
                cigar = get_alignment(p_dict, ref_seq, alt_seq, maxlength)
                print(cigar)
                with open(outputfilename + 'pairwisealignment_%s.txt' % str(chromo), 'a') as outfile:
                    outfile.write("%s, %s\n" % (sanchor, cigar))
            except AssertionError as error:
