...
def get_anchor_intermediate_pos(anchor_pos_19, anchor_pos_37, alignment):
    ...
    for i, (node, pos_19) in enumerate(anchor_pos_19[:-1]):
        ...
        if a == "?":
            ...
            d_037 = anchor_pos_37[i+1, 1] - anchor_pos_19[i, 1]
            d = max(d_019, d_037)
            pos += d
            continue
...
def mapping(poslist, mapping_table, col_index, intermediate = False):
    ...
    Output:
        list of coordinates on the alternative genome
    '''
    mapping_poslist = []
    if intermediate:
        if col_index == 0:
...
def main():
    ...
    t = args.Type[0]
    print(t)
    mapping_coordinates = mapping(poslist, mapping_table, t)
